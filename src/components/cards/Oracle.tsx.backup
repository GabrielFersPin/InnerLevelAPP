  ...
  const handleGenerateCards = async () => {
    ...
    try {
      ...
      switch (generationType) {
        case 'daily':
          const dailyRec = await AIService.getSmartRecommendations(character);
          cards = dailyRec.cards;
          break;
        case 'goal':
          if (goalDescription.trim()) {
            cards = await AIService.generateGoalCards(character, goalDescription, timeframe);
          }
          break;
        case 'situation':
          if (situation.trim()) {
            const situationRec = await AIService.generateClassSpecificCards(character, situation);
