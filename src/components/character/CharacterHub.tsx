...
export function CharacterHub() {
  ...
  const [aiRecommendations, setAiRecommendations] = useState<Card[]>([]);
  ...
  const loadAIRecommendations = async () => {
    setIsLoadingAI(true);
    try {
      const recommendation = await AIService.getSmartRecommendations(character);
  ...
  const displayedRecommendations = showAICards ? aiRecommendations : availableCards.slice(0, 3);
